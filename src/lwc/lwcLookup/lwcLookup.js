...
export default class LwcLookup extends NavigationMixin(BaseElement) {
    // Public
    ...
    validate() {
        this.clearErrors();
        let hasNoError = this.required === true && (this.multiSelect ? (this.value || []).length > 0 : this.value) || this.required !== true || this.required === undefined;
        if (!hasNoError) {
            this.showErrors(['Complete this field.']);
        } else {
            let customError = typeof this.validation === 'object' ? this.validation[this.value] : this.validation;
            hasNoError = !customError;
            if (!hasNoError) {
                this.showErrors([customError]);
            }
        }
        return hasNoError;
    }
    ...
    handleFieldChangeOverridden(event) {
        ...
        if ((this.meta.dto.searchTerm && this.meta.dto.searchTerm.length > 1) || this.showAlRecords == true) {
    ...
    handleRecordOpen(event) {
        if (this.isClickable) {
            if (this.isAndroid) {
                window.open('/' + event.currentTarget.dataset.value || this.value);
            } else {
    ...
    get selectedRecords() {
        let result = this.meta?.dto?.searchResults?.filter(result => (this.value || []).indexOf(result.value) >= 0).map(item => {
            item.invalid = this.validation && this.validation[item.value] !== undefined;
            item.invalidMessage = this.validation ? this.validation[item.value] : undefined;
    ...
    get validation() {
        return this.customValidation || this.multiSelectValidation;
    }
    ...
    get iconName() {
        return (this.iconTag && this.iconTag.startsWith('custom') ? 'custom:' : 'standard:') + this.iconTag || 'default';
    }
